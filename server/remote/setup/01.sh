...
# Prompt to enter a password for the PostgreSQL smphr user (rather than hard-coding a
# password in this script).
read -p "Enter password for smphr DB user: " DB_PASSWORD
...
# Set up the smphr DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE semaphore"
sudo -i -u postgres psql -d semaphore -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d semaphore -c "CREATE ROLE smphr WITH LOGIN PASSWORD '${DB_PASSWORD}'"
sudo -i -u postgres psql -d semaphore -c "GRANT USAGE ON SCHEMA public TO smphr;"
sudo -i -u postgres psql -d semaphore -c "GRANT CREATE ON SCHEMA public TO smphr;"

# Add a DSN for connecting to the semaphore database to the system-wide environment 
# variables in the /etc/environment file.
echo "SMPHR_DSN='postgres://smphr:${DB_PASSWORD}@localhost/semaphore'" >> /etc/environment
